// Vercel Serverless Function — Vicky's brain on the deployed site.
// Mirrors the callVicky() logic in ../serve.mjs so the same /api/vicky-chat
// endpoint works on both localhost:3006 AND the Vercel-deployed dashboard.
//
// Keep this file in sync with serve.mjs if the system prompt or profile
// block changes. Duplication is intentional — Vercel serverless functions
// resolve imports relative to this folder, and the profile string is small
// enough that a shared module would add more complexity than it saves.
...
const MODE_NOTES = {
  ...
  'ask': "Mode: FREE CHAT (floating widget, any page on her dashboard). She's texting you quickly from wherever she is on the dashboard. Answer like a friend, keep it TIGHT — 1-2 sentences usually, she can see the whole reply without scrolling. Reference the page she's on if it's relevant.",
...
function readBody(req) {
  return new Promise((resolve, reject) => {
    // On Vercel the body is auto-parsed into req.body for JSON content-types.
...
async function callVicky(mode, userMessage, dashState, history) {
  ...
  if (!GROQ_KEY) {
    return { ok: false, error: 'no_key', text: "bestie vicky's brain isn't plugged in rn — missing GROQ_API_KEY 🩷" };
  }

  const stateBlock = dashState ? `
## Right now on her dashboard
- Page she's on: ${dashState.page ?? '?'}
...
${dashState.pageContext ? `- Page-specific: ${dashState.pageContext}` : ''}
` : '';

  const modeNote = MODE_NOTES[mode] || MODE_NOTES.ask;
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error('vicky-chat handler error:', e);
